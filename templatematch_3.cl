...
__kernel void template(__global float *d_output,
    ...
    int            template_size,
    float          template_mean,
    __local float *local_sub_img_1d
    )
{
    ...
    if ( image_index_2d.x < nx && image_index_2d.y < ny )
  {
    ...
    for (int c = 0; c < 9; c++)
    {
        int2 load_img_index_2d_clamped = (int 2)(min( nx - 1, max( 0,image_index_2d.x + 2*tx-16+a)), min( ny - 1, max( 0,image_index_2d.y + 2*ty-16+b)));
    ...
    */
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int b = 0; b < 3; b++)
        {
            ...
            local_sub_img_1d[(ty*3+b)*48+(tx*3+a)] = d_inputImage[load_img_index_1d_clamped];
        }
    }
    ...
    for ( int y = -template_half_height; y <= template_half_height; y++ )
    {
        for ( int x = -template_half_width; x <= template_half_width; x++ )
        {
            ...
            float image_offset_value = local_sub_img_1d[(16+ty+y)*48+(16+tx+x)];
